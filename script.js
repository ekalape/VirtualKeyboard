...
function arrowsDiv() {
  ...
  [leftArrBut, rightArrBut, upArrBut, downArrBut].forEach((x) => x.classList.add('button'));
  ...
  [leftArrBut, upArrBut, downArrBut, rightArrBut].forEach((x) => x.addEventListener('click', function onclickArrow(event) {
...
window.addEventListener('keydown', (event) => {
  ...
  if (event.code === 'ShiftLeft' || event.code === 'ShiftRight') {
    ...
    button.forEach((a) => a.classList.add('pressed'));
    writeMe(event.code);
  }
});
window.addEventListener('keyup', (event) => {
  if (event.code === 'ShiftLeft' || event.code === 'ShiftRight') {
    ...
    b.forEach((a) => a.classList.remove('pressed'));
  } else if (['Alt', 'AltLeft', 'AltRight'].includes(event.code)) {
    altdown = false;
    shiftAlt();
    const f = [...base.childNodes].filter(
    ...
    f.forEach((a) => a.classList.remove('pressed'));
  } else if (event.code === 'CapsLock') {
    const cl = [...base.childNodes].filter(
      (x) => x.dataset.code === event.code,
    )[0];
    ...
    button.forEach((a) => a.classList.remove('pressed'));
  }
  textarea.focus();
});
